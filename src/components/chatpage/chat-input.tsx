      ...
      async (e: React.FormEvent) => {
        ...
        if (handleSubmit) {
          ...
          if (attachments.length > 0) {
            ...
            console.log("ChatInput: File being passed:", attachments[0]);
            console.log(
              "ChatInput: File instanceof File:",
              attachments[0] instanceof File
            );

            const file = attachments[0];

            // Check if file can be read as binary (this proves it has binary content)

            // Show first few bytes to prove binary content exists
            file
              .slice(0, 10)
              .arrayBuffer()
              .then((buffer) => {
                ...
                console.log(
                  "🔥 First 10 bytes (proves binary content):",
                  Array.from(bytes)
                );
                console.log(
                  "🔥 Bytes as hex:",
              ...
              .catch((e) =>
                console.log("🔥 Could not read binary content:", e)
              );

            console.log(
              "🔥 IMPORTANT: FormData will send this file in BINARY format!"
            );
            console.log("🔥 The backend will receive the raw binary data!");
            console.log(
              "🔥 Network tab shows 'empty' because browsers don't display binary data!"
            );
            console.log("🔥 ================================");
          }

          console.log(
            "ChatInput: Final options being passed to handleSubmit:",
            options
          );
          console.log(
            "ChatInput: About to call handleSubmit - Stacktrace:",
            new Error().stack
          );

          // Pass to parent handler with both imageUrls and file
          try {
            // 🔍 CRITICAL: Last-chance file validation before sending
            if (options.file) {
              console.log("🔍 FINAL FILE CHECK before API call:");
              console.log(
              ...
              console.log(
                "🔍 File constructor:",
                options.file.constructor.name
              );

              // Check if file has been corrupted
              if (!(options.file instanceof File)) {
                // Try to create a new File object from the corrupted one
                if (
                  typeof options.file === "object" &&
                  "name" in options.file &&
                  "size" in options.file
                ) {
                  console.log("🔧 Attempting to reconstruct File object...");
                  // This won't work perfectly but might help identify the issue
                }
    ...
    const handleAttachmentUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files) {
        ...
        const validFiles = selectedFiles.filter((file) => {
          ...
          const isValidSize = file.size <= 25 * 1024 * 1024; // 25MB limit for documents
